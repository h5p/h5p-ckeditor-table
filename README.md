...
H5P uses this fork of the official CKEditor5 table plugin in the `h5p-editor-php-library` and `h5p-ckeditor` repositories. Follow these steps when working on, or reviewing changes to this repo:

1. (From this folder) `npm link`
2. (From h5p-editor-php-library or h5p-ckeditor) `npm link @h5p/ckeditor5-table`

(H5P.CKEditor (e.g. for FreeTextQuestion) might have to be packed and uploaded instead of updated through a symlink for some setups)
